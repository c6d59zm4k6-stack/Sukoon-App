...
// System prompt for the AI-backed Plan Questionnaire — a faithful port of
// the reference tool's question set and PLAN JSON contract (the human
// shared its source; pcos-root-plan.tiiny.site itself is unreachable from
// this sandbox's network, confirmed 2026-08-24, so this is built from that
// earlier read, not a live re-verification of exact wording). Re-voiced
// for Sukoon (not a single-clinic workshop tool) and taught to use what
// onboarding already collected (name, age, chosen journeys, the "Anything
// we should know?" tags) instead of re-asking for it.
//
// 2026-08-24: restored to the reference's real 14 distinct questions
// (same day, an earlier pass had merged 3 of them down to reduce friction
// -- reverted that merge specifically so this stays a true port, since the
// two goals were in tension). Kept the other half of that pass: explicit
// tag -> question skip rules below, so a symptom already flagged in About
// You's tags is used silently instead of re-asked -- no soft
// acknowledgement either, just skip straight past it. Net effect: a new
// user who skipped every optional tag gets the real, unabridged 14-question
// reference experience; a user who did flag things gets fewer turns
// without the question list itself diverging from the source.
export function buildSystemPrompt(profile = {}) {
  ...
  if (tagSet.has("Weight concerns")) skip.push(`Weight is already flagged as a concern — when you reach Q6, don't ask "do you have stubborn belly weight" from scratch; treat that part as already yes, but still ask about fatigue/cravings since those aren't confirmed yet.`);
...
// hard count. 14 questions, matching the reference tool; a tagged user who
// skips some will reach 100% faster, which is fine for an approximate bar.
