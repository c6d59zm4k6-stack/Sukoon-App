// Manually-synced copy of /conversation/prompt.js from the repo root (the
// original, frozen chat app). Kept as a separate file rather than shared
// across the two independent Vercel projects — see sukoon-webapp's other
// per-feature prompt files (planQuizPrompt.js, doctorConciergePrompt.js) for
// the same pattern. If the root prompt is ever hand-tuned again, re-sync it
// here manually; nothing propagates automatically.
//
...
export function buildConversationPrompt({ memoryFacts, scopeDirective, userContext }) {
  const dynamicParts = [];
  if (userContext) {
