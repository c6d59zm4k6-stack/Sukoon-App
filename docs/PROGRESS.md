...
Onboarding order (`App.jsx` `stage`): **Splash → About You → Choose Journey → main app** (swapped 2026-08-21 at human's request — was Splash → Choose Journey → About You). Both onboarding screens' back buttons were already correctly wired before this change (confirmed live) — only the order changed. Bottom-nav tabs (Home/Plan/Track/Chat/You) have no "back" concept by design; the one real gap was Talk to Experts (see status table above), now fixed.
